...
class WSM_Featured_Post extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		// Replace H2 with H3 for Featured Post Widget Titles - See TKT 772. TKT 1223-Revert titles back to H2
			 echo $before_title . apply_filters( 'widget_title', $instance['title'], $instance, $this->id_base ) . $after_title;
			//echo str_replace( '<h2', '<h3', $before_title ) . apply_filters( 'widget_title', $instance['title'], $instance, $this->id_base ) . str_replace( 'h2>', 'h3>', $after_title );
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			if ( $instance['show_title'] )
				...
				if ( ! empty( $instance['show_title'] ) ) {
					...
					// On front page, want Featured Post titles to be h2 instead of p tags. TKT 772 and via TKT 827 want internal featured post widget entry titles to be p.
					$title_tag = "p";
					if ( is_home() ) $title_tag = "p";
